...
type fastCtxKeyType struct{}

var fastCtxKey = fastCtxKeyType{}
...
func (m *wapcMiddleware) getHandler(metadata middleware.Metadata) (*wapcRequestHandler, error) {
	...
	mod, err := wazero.Engine().New(ctx, m.host, meta.guest, config)
	if err != nil {
...
func (m *wapcMiddleware) host(ctx context.Context, _, namespace, operation string, payload []byte) ([]byte, error) {
	fastCtx := ctx.Value(fastCtxKey).(*fasthttp.RequestCtx)
	switch namespace {
	case "http-handler":
		switch operation {
		case "get-path":
			return fastCtx.Path(), nil
		case "set-path":
			fastCtx.URI().SetPath(string(payload))
		case "next":
...
func (rh *wapcRequestHandler) handle(fastCtx *fasthttp.RequestCtx, instance wapc.Instance) error {
	ctx := context.WithValue(fastCtx, fastCtxKey, fastCtx)
	if _, err := instance.Invoke(ctx, "handle", []byte{}); err != nil {
		return err
	}
	return nil
}
